...
from price_history_route import add_snapshot
import sys

def build_url(origin, destination):
    return f"https://www.google.com/travel/flights?q=Flights%20from%20{origin}%20to%20{destination}&hl=en&curr=USD"
...
def run(origin, destination):
    ...
    with sync_playwright() as p:
            ...
            if not seen_first_real_flight:
                if re.fullmatch(r"\$\d[\d,]*", line):
                    if current:
                        ...
                        has_stop = any(("nonstop" in x.lower()) or ("1 stop" in x.lower()) for x in current)
        ...
        add_snapshot(route_key, parsed)

        browser.close()

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Uso: python collect_prices_dynamic.py LIM BUE")
    else:
